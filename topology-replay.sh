...
if [ "$#" -eq 1 ] && [ "$1" != "--benchmark" ]; then
  ...
  :
elif [ "$#" -eq 2 ] && [ "$1" = "--benchmark-regions" ]; then
  :
elif [ "$#" -eq 2 ] && [ "$1" = "--topology-only" ]; then
  :
  ...
  printf \
    'usage: %s [--benchmark] [--benchmark-regions] [--topology-only] [--manifest <manifest>] <topology-artifact>\n' \
    "$0" >&2
  exit 2
fi
...
OUTPUT_DIR="$ROOT/build/topology-replay"
BINARY="$OUTPUT_DIR/hw-slicer-topology-replay"
