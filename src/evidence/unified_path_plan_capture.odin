...
Unified_Path_Plan_Capture :: struct {
	bytes:      []u8,
	artifact:   Evidence_Artifact,
	additional: Capture_Usage,
}

Unified_Path_Plan_Capture_Error :: enum u8 {
	None,
	Invalid_Request,
	Capture_Disabled,
	Level_Insufficient,
...
unified_path_plan_capture_describe :: proc(
	path: string,
	...
	Unified_Path_Plan_Capture,
	Unified_Path_Plan_Capture_Error,
) {
	...
	if summary.layer_count > max(u64)-summary.path_count ||
	   summary.layer_count+summary.path_count >
		max(u64)-summary.move_count {
		return {}, .Artifact_Limit
	}
	additional := Capture_Usage{
		item_count =
	...
	features.unified_path_plan_artifact_destroy(&decoded, allocator)
	switch decode_error {
	case .None:
	case .Allocation_Failed:
		return {}, .Allocation_Failed
...
unified_path_plan_capture_destroy :: proc(
	capture: ^Unified_Path_Plan_Capture,
	allocator := context.allocator,
) {
