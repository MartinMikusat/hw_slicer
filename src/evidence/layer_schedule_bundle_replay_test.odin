...
@(test)
layer_schedule_bundle_replays_package_and_directory_test :: proc(
	t: ^testing.T,
) {
	artifact_bytes := layer_schedule_capture_test_artifact(t)
	...
	decoded, decode_error :=
		layer_schedule_artifact_decode(artifact_bytes)
	...
	layer_schedule_bundle_expect_replay(t, package_replay)
	...
	destination := directory_test_join(
		t,
		{directory_root, "layer-schedule-evidence"},
	)
	...
	layer_schedule_bundle_expect_replay(t, directory_replay)
}

@(test)
layer_schedule_bundle_dependency_chain_uses_retained_hash_test :: proc(
	t: ^testing.T,
) {
	...
	replay.extrusion.dependencies.layer_schedule_hash[0] = 1
	testing.expect(t, !evidence_bundle_replay_dependencies_valid(replay))
}

layer_schedule_bundle_expect_replay :: proc(
	t: ^testing.T,
	replay: Evidence_Bundle_Replay,
) {
	...
	testing.expect_value(t, len(replay.layer_schedule.result.layer_z), 1)
	testing.expect_value(t, len(replay.layer_schedule.result.layer_ids), 1)
}
