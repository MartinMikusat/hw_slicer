...
@(test)
unified_plan_bundle_replays_package_and_directory_without_source_test :: proc(
	t: ^testing.T,
) {
	artifact_bytes := unified_path_plan_capture_test_artifact(t)
	...
	decoded, decode_error :=
		features.unified_path_plan_artifact_decode(artifact_bytes)
	...
	unified_plan_bundle_expect_replay(t, package_replay)
	...
	destination := directory_test_join(
		t,
		{directory_root, "unified-plan-evidence"},
	)
	...
	unified_plan_bundle_expect_replay(t, directory_replay)
}

@(test)
unified_plan_bundle_dependency_chain_uses_current_plan_test :: proc(
	t: ^testing.T,
) {
	...
	replay.unified_plan.source_paths_hash[0] =
		replay.unified_plan.source_paths_hash[0] ~ 1
	testing.expect(t, !evidence_bundle_replay_dependencies_valid(replay))
	replay.unified_plan.source_paths_hash[0] =
		replay.unified_plan.source_paths_hash[0] ~ 1
...
unified_plan_bundle_expect_replay :: proc(
	t: ^testing.T,
	replay: Evidence_Bundle_Replay,
) {
	...
	testing.expect_value(t, len(replay.unified_plan.result.layers), 1)
	testing.expect_value(t, len(replay.unified_plan.result.paths), 0)
	testing.expect_value(t, len(replay.unified_plan.result.moves), 0)
}
