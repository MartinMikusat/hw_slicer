...
CPU_Intersection_Capture :: struct {
	bytes:      []u8,
	artifact:   Evidence_Artifact,
	additional: Capture_Usage,
}

CPU_Intersection_Capture_Error :: enum u8 {
	None,
	Invalid_Request,
	Capture_Disabled,
	Level_Insufficient,
...
cpu_intersection_capture_describe :: proc(
	path: string,
...
) -> (CPU_Intersection_Capture, CPU_Intersection_Capture_Error) {
	...
	item_count, count_ok :=
		cpu_intersection_capture_item_count(summary)
	...
	decoded, decode_error := cpu_intersection_artifact_decode(
		artifact_bytes,
		limits,
		allocator,
	)
	cpu_intersection_artifact_destroy(&decoded, allocator)
	switch decode_error {
	case .None:
	case .Allocation_Failed:
		return {}, .Allocation_Failed
...
cpu_intersection_capture_item_count :: proc(
	summary: CPU_Intersection_Artifact_Summary,
) -> (u64, bool) {
	result := summary.layer_count
	if result > max(u64)-summary.segment_count {return 0, false}
	result += summary.segment_count
	if result > max(u64)-summary.planar_candidate_count {
		return 0, false
	}
	return result+summary.planar_candidate_count, true
}

cpu_intersection_capture_destroy :: proc(
	capture: ^CPU_Intersection_Capture,
	allocator := context.allocator,
) {
