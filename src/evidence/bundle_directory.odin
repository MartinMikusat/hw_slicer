...
evidence_bundle_directory_validate :: proc(
	...
	if manifest_error != .None {return .Invalid_Manifest}
	defer evidence_bundle_manifest_destroy(&manifest, allocator)
	if !evidence_bundle_directory_manifest_fits_limits(
		manifest,
		u64(len(root_manifest_bytes)),
		limits,
	...
	node_count: u64
	node_limit := evidence_bundle_directory_expected_node_limit(manifest)
	...
	for descriptor in manifest.files {
		bytes, read_error := evidence_bundle_directory_read_file(
			root_fd,
			descriptor.path,
		...
		verify_error := evidence_artifact_verify(descriptor, bytes)
		delete(bytes, allocator)
		if verify_error != .None {return .Invalid_Content}
	}
	summary_bytes, summary_error := evidence_bundle_directory_read_file(
		root_fd,
		manifest.summary.path,
	...
	if summary_error != .None {return summary_error}
	if evidence_artifact_verify(manifest.summary, summary_bytes) != .None {
		delete(summary_bytes, allocator)
		return .Invalid_Content
	}
	summary_matches :=
		evidence_bundle_summary_matches(manifest, summary_bytes, allocator)
	delete(summary_bytes, allocator)
	if !summary_matches {return .Invalid_Content}
	for stage in manifest.stages {
		stage_bytes, stage_error := evidence_bundle_directory_read_file(
			root_fd,
			stage.manifest.path,
		...
		if evidence_artifact_verify(stage.manifest, stage_bytes) != .None {
		...
		if stage.stage.name == "plan-paths" ||
		   stage.stage.name == "reconstruct-topology" ||
		   stage.stage.name == "calculate-regions" {
			stage_manifest, decode_error :=
				evidence_manifest_decode(stage_bytes, allocator)
			if decode_error != .None {
				delete(stage_bytes, allocator)
				return .Invalid_Content
			}
			primitive_format := PATH_PLAN_ARTIFACT_FORMAT
			if stage.stage.name == "reconstruct-topology" {
				primitive_format = TOPOLOGY_ARTIFACT_FORMAT
			} else if stage.stage.name == "calculate-regions" {
				primitive_format = REGION_ARTIFACT_FORMAT
			}
			...
			if !primitive_ok {
				evidence_manifest_destroy(&stage_manifest, allocator)
				delete(stage_bytes, allocator)
				return .Invalid_Content
			}
			...
			semantic_error := Evidence_Bundle_Package_Error.None
			if stage.stage.name == "plan-paths" {
				semantic_error =
					evidence_bundle_path_plan_manifest_validate(
						stage_manifest,
						artifact_bytes,
						allocator,
					)
			} else if stage.stage.name == "reconstruct-topology" {
				semantic_error =
					evidence_bundle_topology_manifest_validate(
						stage_manifest,
						artifact_bytes,
						allocator,
					)
				...
				topology, topology_error :=
					evidence_bundle_directory_topology_dependency_decode(
						root_fd,
						manifest,
						allocator,
					)
				...
				semantic_error =
					evidence_bundle_region_manifest_validate(
						stage_manifest,
						artifact_bytes,
						topology,
						allocator,
					)
				topology_artifact_destroy(&topology, allocator)
			}
			delete(artifact_bytes, allocator)
			evidence_manifest_destroy(&stage_manifest, allocator)
			if semantic_error != .None {
				delete(stage_bytes, allocator)
				if semantic_error == .Allocation_Failed {
					return .Allocation_Failed
				}
				return .Invalid_Content
			}
	...
	root_unchanged :=
		...
		evidence_bundle_bytes_equal(root_manifest_after, root_manifest_bytes)
	delete(root_manifest_after, allocator)
	if !root_unchanged {return .Invalid_Manifest}
	node_count = 0
	return evidence_bundle_directory_walk_validate(
		root_fd,
		"",
		manifest,
		&node_count,
...
evidence_bundle_directory_topology_dependency_decode :: proc(
	root_fd: posix.FD,
	root: Evidence_Bundle_Manifest,
	allocator := context.allocator,
) -> (Topology_Artifact, Evidence_Bundle_Directory_Error) {
	...
	manifest_bytes, manifest_read_error :=
		evidence_bundle_directory_read_file(
			...
			topology_stage.manifest.byte_count,
			topology_stage.manifest.byte_count,
			allocator,
		)
	if manifest_read_error != .None {return {}, manifest_read_error}
	defer delete(manifest_bytes, allocator)
	if evidence_artifact_verify(
		topology_stage.manifest,
		manifest_bytes,
	) != .None {
		return {}, .Invalid_Content
	}
	...
	if artifact_read_error != .None {return {}, artifact_read_error}
