...
Motion_Plan_Capture :: struct {
	bytes:      []u8,
	artifact:   Evidence_Artifact,
	additional: Capture_Usage,
}

Motion_Plan_Capture_Error :: enum u8 {
	None,
	Invalid_Request,
	Capture_Disabled,
	Level_Insufficient,
...
motion_plan_capture_describe :: proc(
	path: string,
...
) -> (Motion_Plan_Capture, Motion_Plan_Capture_Error) {
	...
	if summary.layer_count > max(u64)-summary.operation_count {
		return {}, .Artifact_Limit
	}
	additional := Capture_Usage{
		item_count = summary.layer_count+summary.operation_count,
	...
	decoded, decode_error := features.motion_plan_artifact_decode(
		artifact_bytes,
		limits,
		allocator,
	)
	features.motion_plan_artifact_destroy(&decoded, allocator)
	switch decode_error {
	case .None:
	case .Allocation_Failed:
		return {}, .Allocation_Failed
...
motion_plan_capture_destroy :: proc(
	capture: ^Motion_Plan_Capture,
	allocator := context.allocator,
) {
