package evidence

import contracts "../contracts"
import slicing "../slicing"

LAYER_SCHEDULE_MANIFEST_STAGE_REVISION :: u32(1)

Layer_Schedule_Manifest_Expectations :: struct {
	layer_count: u64,
	result_hash: contracts.Content_Hash,
}

Layer_Schedule_Manifest_Error :: enum u8 {
	None,
	Invalid_Manifest,
	Stage_Mismatch,
	Artifact_Missing,
	Artifact_Format_Mismatch,
	Artifact_Byte_Count_Mismatch,
	Artifact_Hash_Mismatch,
	Summary_Mismatch,
	Invariant_Mismatch,
	Result_Mismatch,
}

layer_schedule_manifest_preflight :: proc(
	manifest: Evidence_Manifest,
	artifact_path: string,
	artifact_bytes: []u8,
	limits := DEFAULT_LAYER_SCHEDULE_ARTIFACT_LIMITS,
) -> (
	Layer_Schedule_Manifest_Expectations,
	Layer_Schedule_Manifest_Error,
) {
	if !evidence_manifest_valid(manifest) {
		return {}, .Invalid_Manifest
	}
	if manifest.stage.name != "schedule-layers" ||
	   manifest.stage.schema_version !=
		slicing.SCHEMA_VERSION_FIXED_LAYER_SCHEDULE_HASH ||
	   manifest.stage.revision !=
		LAYER_SCHEDULE_MANIFEST_STAGE_REVISION {
		return {}, .Stage_Mismatch
	}
	descriptor: Evidence_Artifact
	descriptor_found := false
	for primitive in manifest.primitives {
		if primitive.path == artifact_path {
			descriptor = primitive
			descriptor_found = true
			break
		}
	}
	if !descriptor_found {return {}, .Artifact_Missing}
	if descriptor.format != LAYER_SCHEDULE_ARTIFACT_FORMAT ||
	   descriptor.schema_version !=
		LAYER_SCHEDULE_ARTIFACT_SCHEMA_VERSION {
		return {}, .Artifact_Format_Mismatch
	}
	switch evidence_artifact_verify(descriptor, artifact_bytes) {
	case .None:
	case .Invalid_Descriptor:
		return {}, .Invalid_Manifest
	case .Byte_Count_Mismatch:
		return {}, .Artifact_Byte_Count_Mismatch
	case .Hash_Mismatch:
		return {}, .Artifact_Hash_Mismatch
	}
	artifact_summary, artifact_error :=
		layer_schedule_artifact_preflight(artifact_bytes, limits)
	if artifact_error != .None ||
	   descriptor.item_count != artifact_summary.layer_count {
		return {}, .Summary_Mismatch
	}
	expectations := Layer_Schedule_Manifest_Expectations{
		layer_count = artifact_summary.layer_count,
	}
	layer_count, layer_count_found :=
		path_plan_manifest_counter(manifest, "scheduled_layers")
	if !layer_count_found || layer_count != expectations.layer_count {
		return {}, .Summary_Mismatch
	}
	result_invariant, result_invariant_ok :=
		path_plan_manifest_invariant(
			manifest,
			"canonical_layer_schedule_result_hash",
		)
	if !result_invariant_ok ||
	   !result_invariant.passed ||
	   result_invariant.observed != result_invariant.expected {
		return {}, .Invariant_Mismatch
	}
	expectations.result_hash, result_invariant_ok =
		path_plan_manifest_hash_parse(result_invariant.expected)
	if !result_invariant_ok {return {}, .Invariant_Mismatch}
	replay_invariant, replay_invariant_ok :=
		path_plan_manifest_invariant(
			manifest,
			"source_independent_layer_schedule_replay",
		)
	if !replay_invariant_ok ||
	   !replay_invariant.passed ||
	   replay_invariant.observed != "passed" ||
	   replay_invariant.expected != "passed" {
		return {}, .Invariant_Mismatch
	}
	return expectations, .None
}

layer_schedule_manifest_replay_verify :: proc(
	expectations: Layer_Schedule_Manifest_Expectations,
	artifact: Layer_Schedule_Artifact,
) -> Layer_Schedule_Manifest_Error {
	if u64(len(artifact.result.layer_z)) != expectations.layer_count ||
	   u64(len(artifact.result.layer_ids)) != expectations.layer_count ||
	   artifact.result_hash != expectations.result_hash {
		return .Result_Mismatch
	}
	return .None
}
