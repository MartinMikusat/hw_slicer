...
@(test)
unified_source_bundle_replays_package_and_directory_without_source_test :: proc(
	t: ^testing.T,
) {
	artifact_bytes := unified_path_source_capture_test_artifact(t)
	...
	decoded, decode_error :=
		features.unified_path_source_artifact_decode(artifact_bytes)
	...
	unified_source_bundle_expect_replay(t, package_replay)
	...
	destination := directory_test_join(
		t,
		{directory_root, "unified-source-evidence"},
	)
	...
	unified_source_bundle_expect_replay(t, directory_replay)
}

unified_source_bundle_expect_replay :: proc(
	t: ^testing.T,
	...
	testing.expect_value(t, len(replay.unified_sources.result.layers), 1)
	testing.expect_value(t, len(replay.unified_sources.result.sources), 0)
	testing.expect_value(t, len(replay.unified_sources.result.points), 0)
}
