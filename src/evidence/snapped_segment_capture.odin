...
Snapped_Segment_Capture :: struct {
	bytes:      []u8,
	artifact:   Evidence_Artifact,
	additional: Capture_Usage,
}

Snapped_Segment_Capture_Error :: enum u8 {
	None,
	Invalid_Request,
	Capture_Disabled,
	Level_Insufficient,
...
snapped_segment_capture_describe :: proc(
	path: string,
...
) -> (Snapped_Segment_Capture, Snapped_Segment_Capture_Error) {
	...
	item_count, count_ok :=
		snapped_segment_capture_item_count(summary)
	...
	decoded, decode_error := snapped_segment_artifact_decode(
		artifact_bytes,
		limits,
		allocator,
	)
	snapped_segment_artifact_destroy(&decoded, allocator)
	switch decode_error {
	case .None:
	case .Allocation_Failed:
		return {}, .Allocation_Failed
...
snapped_segment_capture_item_count :: proc(
...
snapped_segment_capture_destroy :: proc(
	capture: ^Snapped_Segment_Capture,
	allocator := context.allocator,
) {
