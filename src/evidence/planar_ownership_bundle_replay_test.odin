...
@(test)
planar_ownership_bundle_replays_package_and_directory_test :: proc(
	t: ^testing.T,
) {
	result := planar_ownership_artifact_test_result()
	defer slicing.planar_ownership_destroy(&result)
	artifact_bytes, encode_error := planar_ownership_artifact_encode(
	...
	decoded, decode_error :=
		planar_ownership_artifact_decode(artifact_bytes)
	...
	planar_ownership_bundle_expect_replay(t, package_replay)
	...
	destination := directory_test_join(
		t,
		{directory_root, "planar-ownership-evidence"},
	)
	...
	planar_ownership_bundle_expect_replay(t, directory_replay)
}

@(test)
planar_ownership_bundle_dependency_chain_uses_retained_graph_test :: proc(
	t: ^testing.T,
) {
	replay := Evidence_Bundle_Replay{
		intersections_loaded = true,
		planar_ownership_loaded = true,
		snapped_loaded = true,
	}
	...
	replay.planar_ownership.result.layers =
	...
	replay.planar_ownership.intersection_hash[0] += 1
	testing.expect(t, !evidence_bundle_replay_dependencies_valid(replay))
	replay.planar_ownership.intersection_hash[0] -= 1
	replay.planar_ownership.result.layers =
		replay.planar_ownership.result.layers[:1]
	testing.expect(t, !evidence_bundle_replay_dependencies_valid(replay))
	replay.planar_ownership.result.layers =
		[]slicing.Snapped_Layer{{}, {}}
	replay.snapped.parent_hash[0] += 1
	testing.expect(t, !evidence_bundle_replay_dependencies_valid(replay))
}

planar_ownership_bundle_expect_replay :: proc(
	t: ^testing.T,
	replay: Evidence_Bundle_Replay,
) {
	testing.expect(t, replay.planar_ownership_loaded)
	...
	testing.expect_value(
		t,
		len(replay.planar_ownership.result.segments.segment_ids),
		2,
	)
	testing.expect_value(
		t,
