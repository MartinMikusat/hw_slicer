...
@(test)
infill_bundle_replays_package_and_directory_without_source_test :: proc(
	t: ^testing.T,
) {
	artifact_bytes := infill_capture_test_artifact(t)
	defer delete(artifact_bytes)
	artifact_path := "stages/09-generate-features/primitives/infill.bin"
	...
	testing.expect_value(
		t,
		capture_error,
		Infill_Capture_Error.None,
	)
	decoded, decode_error :=
		features.infill_artifact_decode(artifact_bytes)
	...
	infill_bundle_expect_replay(t, package_replay)
	...
	destination := directory_test_join(
		t,
		{directory_root, "infill-evidence"},
	)
	...
	infill_bundle_expect_replay(t, directory_replay)
}

@(test)
infill_bundle_dependency_chain_uses_retained_hashes_test :: proc(
	t: ^testing.T,
) {
	...
	replay.regions.result_hash = INFILL_CAPTURE_TEST_REGION_HASH
	replay.infill.region_hash = INFILL_CAPTURE_TEST_REGION_HASH
	...
	replay.infill.region_hash[0] = replay.infill.region_hash[0] ~ 1
	testing.expect(t, !evidence_bundle_replay_dependencies_valid(replay))
	replay.infill.region_hash[0] = replay.infill.region_hash[0] ~ 1
	replay.unified_sources.dependencies.infill_hash[0] = 1
	testing.expect(t, !evidence_bundle_replay_dependencies_valid(replay))
}

infill_bundle_expect_replay :: proc(
	t: ^testing.T,
	replay: Evidence_Bundle_Replay,
) {
	...
	testing.expect_value(t, len(replay.infill.result.layers), 1)
	testing.expect_value(t, len(replay.infill.result.segments), 0)
	testing.expect_value(t, len(replay.infill.result.boundary_hits), 0)
}
