...
Infill_Capture :: struct {
	bytes:      []u8,
	artifact:   Evidence_Artifact,
	additional: Capture_Usage,
}

Infill_Capture_Error :: enum u8 {
	None,
	Invalid_Request,
	Capture_Disabled,
	Level_Insufficient,
...
infill_capture_describe :: proc(
...
) -> (Infill_Capture, Infill_Capture_Error) {
	...
	if summary.layer_count > max(u64)-summary.segment_count ||
	   summary.layer_count+summary.segment_count >
		max(u64)-summary.hit_count {
		return {}, .Artifact_Limit
	}
	additional := Capture_Usage{
		item_count =
	...
	decoded, decode_error := features.infill_artifact_decode(
		artifact_bytes,
		limits,
		allocator,
	)
	features.infill_artifact_destroy(&decoded, allocator)
	switch decode_error {
	case .None:
	case .Allocation_Failed:
		return {}, .Allocation_Failed
...
infill_capture_destroy :: proc(
	capture: ^Infill_Capture,
	allocator := context.allocator,
) {
